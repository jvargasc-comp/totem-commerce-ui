...
const stylesFor = (variant: Variant, disabled?: boolean): React.CSSProperties => {
  ...
  switch (variant) {
    ...
    case "secondary":
      return { ...base, background: "var(--surface-2)", color: "var(--text)", borderColor: "rgba(233,238,246,.10)" };
    case "danger":
      return { ...base, background: "var(--danger)", color: "white" };
    case "ghost":
    default:
      return { ...base, background: "transparent", color: "var(--text)", borderColor: "rgba(233,238,246,.18)", boxShadow: "none" };
  }
};
