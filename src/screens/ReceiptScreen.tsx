...
export default function ReceiptScreen({ orderId, onNew }: Props) {
  ...
  // ✅ Envío real (DHL simulado) persistido en backend
  // Extend Receipt type to include optional shipping fields
  type ReceiptWithShipping = Receipt & {
  ...
  const shippingCents = useMemo(() => {
    if (!receipt) return 0;
    const r = receipt as ReceiptWithShipping;
    const sc = typeof r.shippingCents === "number" ? r.shippingCents : 0;
    return sc;
  }, [receipt]);

  const shippingLabel = useMemo(() => {
    if (!receipt) return "Envío";
    const r = receipt as ReceiptWithShipping;
    const sp = String(r.shippingProvider ?? "").toUpperCase();
    if (sp.includes("DHL")) return "Envío DHL";
    return "Envío";
  }, [receipt]);
          ...
          <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
            ...
            >
              ...
              <div style={{ display: "grid", gap: 8, fontSize: 16 }}>
                ...
                {/* ✅ aquí cambiamos a shippingCents */}
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ flex: 1, opacity: 0.85 }}>{shippingLabel}</div>
                  <div style={{ fontWeight: 900 }}>{money(shippingCents)}</div>
                </div>
            ...
            >
              ...
              >
                <div>
                  <span style={{ opacity: 0.8 }}>Proveedor:</span> <b>{receipt.payment?.provider ?? "SIMULATED"}</b>
                </div>
                <div>
                  <span style={{ opacity: 0.8 }}>Estado:</span> <b>{receipt.payment?.status ?? "INITIATED"}</b>
                </div>
                <div>
                  <span style={{ opacity: 0.8 }}>Monto:</span>{" "}
                  <b>
                    {money(receipt.payment?.amountCents ?? receipt.totalCents)} {receipt.payment?.currency ?? "USD"}
                  </b>
                </div>
